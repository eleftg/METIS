          ...
          idx_t *options, idx_t *perm, idx_t *iperm)
{
  ...
  int numflag = 0;
#if IDXTYPEWIDTH == 32
  char *  envswitch = getenv( "ELS_ORD_FORCE_METIS" ) ;
#endif
  ...
  if (envswitch)
  {
    IFSET(ctrl->dbglvl, METIS_DBG_INFO,printf("In METIS_NodeND : force ND activated by env var ELS_ORD_FORCE_METIS\n"));
  }
#endif
...
#if IDXTYPEWIDTH == 32
  if (lgraph->nvtxs > MMDSWITCH && (lgraph->nedges > 0 || envswitch))
...
#if IDXTYPEWIDTH == 32
  if (rgraph->nvtxs > MMDSWITCH && (rgraph->nedges > 0 || envswitch))
#elif IDXTYPEWIDTH == 64
...
#if IDXTYPEWIDTH == 32
    if (sgraphs[i]->nvtxs > MMDSWITCH && (sgraphs[i]->nedges > 0 || envswitch)) {
#elif IDXTYPEWIDTH == 64
